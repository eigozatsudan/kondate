...
describe("MenuResultPage", () => {
  ...
  it("keeps create sticky on safety_fingerprint_changed for same-key resume (SHOP2)", async () => {
    // replay 中の FP before/after ずれは current_safety ではなく safety_fingerprint_changed。
    // sticky を捨てると mode=new 再送が dual-create になる。
    getMenuResultMock.mockResolvedValue(makeMenuResultViewModel());
    shoppingApi.createShoppingList.mockRejectedValue(
      Object.assign(new Error("家族設定が変わったため、もう一度確認してください"), {
