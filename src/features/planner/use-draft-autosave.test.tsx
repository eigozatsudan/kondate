...
it("競合後にサーバーの新しい revision が届くと自動保存を再開する", async () => {
  vi.useFakeTimers();
  const conflict = new DraftRevisionConflictError();
  const stale = { ...base, mealType: "dinner" as const, memo: "Aの入力" };
  const resynced = { ...base, mealType: "lunch" as const, memo: "最新の下書き" };
  const save = vi
    .fn<(value: PlannerDraftInput, revision: number) => Promise<PlannerDraft>>()
    .mockRejectedValueOnce(conflict)
    .mockImplementation((value, revision) => Promise.resolve(saved(value, revision + 1)));
  const { rerender, result } = renderHook(
    ({ value, initialRevision }) =>
      useDraftAutosave({ value, enabled: true, initialRevision, save }),
    { initialProps: { value: base, initialRevision: 1 } },
  ...
  // 呼び出し側が競合後の refetch でサーバー最新の下書きと revision を反映した状態を模す。
  rerender({ value: resynced, initialRevision: 2 });
  expect(result.current.state).toBe("idle");
  expect(result.current.revision).toBe(2);

  const editedAfterResync = { ...resynced, budgetPreference: "economy" as const };
  rerender({ value: editedAfterResync, initialRevision: 2 });
  await act(async () => vi.advanceTimersByTimeAsync(600));

  expect(save).toHaveBeenCalledTimes(2);
  expect(save).toHaveBeenNthCalledWith(2, editedAfterResync, 2);
