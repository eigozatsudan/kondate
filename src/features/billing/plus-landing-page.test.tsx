...
describe("PlusLandingPage entitled benefits and period", () => {
  ...
  // UX 残り R3 M-2（ユーザー決定）: 期間末は申込の時刻にそろうので、日中に終わるなら時刻まで示す
  it("shows the JST time of day when the period ends during the day", () => {
    renderLp({
      entitlement: {
        ...plusActive,
        currentPeriodEnd: "2026-10-23T05:32:11.000Z",
        cancelAtPeriodEnd: true,
      },
      now: FIXED_NOW,
    });
    expect(
      screen.getByText("2026年10月23日 14:32 に Plus が終了します（自動更新なし）"),
    ).toBeVisible();
  });
  ...
  it("shows the trial end as the Plus end date when cancellation is scheduled during the trial", () => {
    ...
    expect(
      screen.getByText("2026年10月1日 00:00 に Plus が終了します（自動更新なし）"),
    ).toBeVisible();
  ...
  // その前に更新・課金が起きるので終了予定として扱わず、従来の表示を出す
  it("keeps the renewal date when cancel_at is after the current period end", () => {
    renderLp({
      entitlement: {
        ...plusActive,
        currentPeriodEnd: "2026-10-22T15:00:00.000Z",
        cancelAtPeriodEnd: false,
        cancelAt: "2026-12-09T15:00:00.000Z",
      },
      now: FIXED_NOW,
    });
    expect(screen.getByText("次回の更新日: 2026年10月23日")).toBeVisible();
  ...
  it("keeps the trial charge warning when cancel_at is after the trial end", () => {
    renderLp({
      entitlement: {
        ...
        trialEnd: "2026-09-30T15:00:00.000Z",
        cancelAtPeriodEnd: false,
        cancelAt: "2026-11-14T15:00:00.000Z",
      },
      now: FIXED_NOW,
    });
    expect(screen.getByText("無料期間の終了: 2026年10月1日")).toBeVisible();
