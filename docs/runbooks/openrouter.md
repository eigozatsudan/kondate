...
- 各設定 ID は Models API 上で `structured_outputs` **AND** `response_format` を公開し、`pricing.prompt` + `pricing.completion` ≤ **$6.00 / 1M tokens** であること。
...
3. prompt+completion が $6.00/1M 以下であることを確認する。
