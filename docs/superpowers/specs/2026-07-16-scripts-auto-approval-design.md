...
ルールは上記のリテラルな引数列に一致し、後続引数はexecpolicyのprefixルール仕様に従って許可する。`prefix_rule` は実行時のカレントディレクトリやリポジトリルートを判定しないため、リポジトリ直下だけには限定されず、別のカレントディレクトリにある同じ `./scripts/...` 形式にも一致する。
...
- 絶対パスや `./scripts/../scripts/run-e2e.sh` のように、許可した文字列と異なる字句形式での実行
...
相対パスの `prefix_rule` はカレントディレクトリを拘束しないため、別のカレントディレクトリに同じ `./scripts/...` のパスを用意した場合も自動承認される。また、許可済みのパス名をシンボリックリンクへ置き換えた場合、ルールは同じ引数列へ一致したままリンク先を実行する。ユーザーは2026-07-16にこの2つの残存リスクを明示的に受容し、相対パスのルールを維持する選択肢を選んだ。
...
- `./scripts/../scripts/run-e2e.sh` が追加ルールに一致しない。
