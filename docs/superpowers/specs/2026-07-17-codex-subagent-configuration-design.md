...
4. 新しいCodexプロセスで、検証開始時の希望する通常permissionを記録してから、親のlive runtime permissionとしてread-onlyを選択し、実効状態を確認する。確認後にだけ `explorer`、`reviewer`、`fast-worker` をspawnし、各custom agentのload、固有の役割指示、リポジトリ書き込み拒否を観測する。可能なら書き込み可能なlive overrideでも各developer instructionが編集を拒否することを別途確認する。
5. `fast-worker` に、リポジトリルートをcwdとして `docker compose run --rm --no-deps app node -p 'JSON.stringify({execPath:process.execPath,cwd:process.cwd(),version:process.version})'` と、host-nativeの `git rev-parse --show-toplevel` をそれぞれ独立して実行させる。両方の終了コード0を必須とし、正確なコマンド、Docker経由／host-nativeの実行経路、呼出しcwd、標準出力の要約を記録する。追跡対象ファイルの差分を残さない。
...
7. Implementer検証後は、検証開始時に記録した希望する通常permissionへ明示的に戻して実効状態を確認する。開始時、read-only選択、workspace-capable選択、通常permission復元の各遷移と確認結果を記録する。4種類のうち1種類でも未観測、いずれかのpermissionを選択・確認できない、またはworktreeの排他性を確認できない場合は完全検証済みとせず、未解決制約として記録する。
...
11. `AGENTS.md`、`SubAgents.md`、各 TOML の役割と権限が矛盾しないこと。特にReviewerの根拠一覧、`fast-worker`の`:read-only`、live override警告、Docker適用範囲をfocused `rg` で確認する。
