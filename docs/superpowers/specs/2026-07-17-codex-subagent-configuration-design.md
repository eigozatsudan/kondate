...
- リポジトリを書き込む Implementer は常に 1 体だけとし、並列書き込みを禁止する。これはCodex設定によるcustom agent種別ごとの技術的な同時数制限ではなく、親が維持するオーケストレーション不変条件である。
- 親はImplementerをdispatchする直前にactive agent threadを確認する。既存Implementerがactiveなら、そのImplementerが完了またはcloseされるまで2体目をdispatchしない。active状態を確認できない場合はdispatchを停止し、未確認の制約として報告する。
...
- 制約: このTOML自体はsingleton動作を技術的に強制しない。Implementerは編集前に、利用可能なagent状態と親のdispatch情報から自分が唯一のactive Implementerであることを確認する。排他性が不確実、または別のactive Implementerがいる場合は編集を停止して親へ報告する。Task brief と対象ファイルの範囲を守り、仕様やロック済みインターフェースを独自に変更しない。
...
- single-writerはCodexの役別上限ではなく親が維持するオーケストレーション不変条件であり、親がdispatch前にactive agent threadを確認し、既存Implementerの完了またはclose前に2体目を起動せず、状態を確認できなければ停止すること。
...
4. 新しいCodexプロセスで `explorer`、`reviewer`、`fast-worker`、`implementer` の4種類すべてを実際にspawnし、各custom agentのloadと固有の役割指示を観測する。読み取り専用3役は親のlive runtime permissionとしてread-onlyを選択した状態でリポジトリ書き込みが拒否されることを確認し、可能なら書き込み可能なlive overrideでも各developer instructionが編集を拒否することを確認する。Implementerは他のImplementerがactiveでない状態だけで起動し、親model/reasoningの継承と`:workspace`の実効値を、承認されたdisposable worktree上のsentinel操作など安全な方法で観測する。4種類のうち1種類でも未観測なら完全検証済みとせず、未解決制約として記録する。
