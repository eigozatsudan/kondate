...
**Interfaces:**
- Consumes: Plan 2's `loadCurrentSafetyContext(admin,userId,targetMemberIds)`, nullable owner-composite `menu_target_members.(household_member_id,household_member_user_id)` link, immutable `member_display_name_snapshot`/`anonymous_ref`, normalized `menu_safety_actions`, `createCurrentSafetyFingerprint(context)`, distinct `validatedMenuSchema` / `generatedMenuSchema`, `ValidatedMenu` / `GeneratedMenu`, and Plan 3's `createUserScopedSupabase(accessToken)`. This task owns its normalized server-side stored-menu loader and dedicated `validateStoredMenuCurrentSafety` subset validator; it never imports Plan 3's browser-only `getMenuResult()` or trusts a service-role query to establish ownership.
- Produces: `loadStoredMenu(client,userId,menuId)`, `toStoredRevalidationCandidate(menu,currentContext)`, `revalidateStoredMenu(deps,input)`, `createRevalidationDeps(user)`, and `POST /api/menus/:menuId/revalidate`. Stored confirmation provenance remains display/audit data; only a current-canonical pending generated-shape projection enters the provider-output validator. The response also carries the exact current human-readable label-warning projection so a newly added allergy cannot pass revalidation while remaining invisible on the result page.
...
export async function loadStoredMenu(
  ...
  const targetMembers=data.menu_target_members.map((item)=>({
...
Drop/revoke Plan 2's two-argument confirmation RPC and create `public.confirm_menu_label_confirmation(p_menu_id uuid,p_confirmation_id uuid,p_expected_safety_fingerprint text)`. It locks/recomputes current safety through the same private function and updates only an owner row with `is_current`, `requirement_safety_fingerprint = p_expected_safety_fingerprint`, and `pending`; stale/foreign/already-confirmed IDs remain the same empty result. Grant only this three-argument overload to `authenticated`. pgTAP proves a settings change makes the old ID unconfirmable, reconciliation creates new pending IDs, same-fingerprint replay preserves a confirmed row, a concurrent fingerprint change rolls back reconciliation/confirmation, and another owner is indistinguishable.

Implement exact helper `reconcileCurrentMenuLabelWarnings(admin,user,input)` in `revalidation-adapter.ts` from the already validated `candidate.labelConfirmations`, and bind it as the dependency's `reconcileCurrentLabelWarnings`. Send only the closed exact key fields to that RPC, parse its returned IDs/status/fingerprint, and then resolve `sourceText` through `collectMenuTextSources(candidate)`, allergen names through the current catalog, and member names by live name→immutable snapshot→`家族N`. Every returned current warning therefore has a non-null `confirmationId`. Cap and sort by source path, allergen ID, and anonymous member ref. Tests cover two leaves sharing one normalized source ID, an added allergy creating a confirmable pending warning, a removed allergy archiving the obsolete warning, same-fingerprint confirmed provenance, deleted-member snapshot fallback, and no raw UUID/catalog ID/source path in displayed copy. Historical rows never enter validator evidence merely because they remain stored for audit.
