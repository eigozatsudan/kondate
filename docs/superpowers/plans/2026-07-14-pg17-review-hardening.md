...
Expected: ignore patterns、絶対Compose path、`sh -eu -c`、正の`COMPOSE_FILE`拒否、refresh wrapper参照が未実装のためFAIL。
...
Supabase更新手順はTask 3で追加する`./scripts/refresh-supabase.sh`だけを案内する。
