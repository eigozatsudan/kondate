...
**Architecture:** menu-detail の純関数 `buildMenuIngredientsSummary` が dishes を売り場区分順の合算行に変換する。正規化は `@shared/shopping/normalize` を再利用し、在庫差し引きと aliases は使わない。`MenuSteps` にローカルタブ state を置き、両適用面（生成結果・`/menus/:id`）に同時に載せる。`categoryLabel` は `src/shared/ui` へ移し feature 間依存を避ける。既存 E2E / 単体のスコープなし `tab`/`tabpanel` クエリを料理 tablist 配下に直す。

**Tech Stack:** React 19、TypeScript strict、Vitest + Testing Library、Playwright E2E、既存 `.menu-result-*` / `.cook-timeline-*` 意味クラス、Docker `app` 経由の検証。
...
| ファイル | 責務 |
| --- | --- |
| `src/shared/ui/store-section-label.ts` | **新規** `categoryLabel` 正本 |
...
| `src/features/menu-detail/menu-ingredients-summary.tsx` | **新規** 区分+行表示 |
| `src/features/menu-detail/menu-ingredients-summary.test.tsx` | **新規** 表示テスト |
| `src/features/menu-detail/menu-steps.tsx` | タブ + panel 切替 |
| `src/features/menu-detail/menu-steps.test.tsx` | タブ a11y / 初期段取り |
| `src/features/generation/components/menu-result.test.tsx` | 料理 tab スコープ修正 |
...
`src/shared/ui/store-section-label.ts`:
...
export function buildMenuIngredientsSummary(
  ...
  for (const d of orderedDishes) {
    ...
    for (const item of orderedIngredients) {
      ...
      const mergeable = item.quantityValue !== null && unit !== null;
      const key = mergeable
        ? JSON.stringify(["m", normalizedName, unit])
        : JSON.stringify(["a", normalizedName, unit, item.quantityText]);
      const existing = groups.get(key);
      if (existing === undefined) {
        groups.set(key, {
          key,
          displayName: item.name,
          quantityValue: mergeable ? item.quantityValue : null,
          quantityText: mergeable
            ? `${formatQuantityValue(item.quantityValue!)}${unit}`
            : item.quantityText,
          unit,
      ...
      } else if (mergeable && existing.quantityValue !== null && item.quantityValue !== null) {
        const sum = roundQuantityValue(existing.quantityValue + item.quantityValue);
...
（実装時に non-null 断言を避け、`quantityValue` の narrowing を strict に書くこと。）
...
Surface > Inset > .cook-timeline-panel
  ...
  > role=tablist aria-label="献立の段取りと材料" class=menu-result-tabs
...
it("renders store-section headings and ingredient rows from dishes", () => {
  ...
  const produce = screen.getByRole("heading", { name: "野菜" }).closest("section");
  if (!(produce instanceof HTMLElement)) throw new Error("produce section required");
...
/**
 * 献立全体の材料まとめ（表示専用）。
 * 合算は buildMenuIngredientsSummary。生 Tailwind 禁止 → 意味クラスのみ。
 */
    ...
    <Stack gap={4}>
      {sections.map((section) => (
        <section
          ...
          aria-labelledby={`ingredient-section-${section.storeSection}`}
        >
          <h3
...
- tab ボタン: `className="menu-result-tab"`、list: `className="menu-result-tabs"`
...
| 箇所 | 変更 |
| --- | --- |
| `getByRole("tab", { name: new RegExp(secondDish.name) })` | `within(dishTablist()).getByRole("tab", { name: ... })` |
| `getByRole("tabpanel")` | `selectedDishPanel()` |
| `getAllByRole("tab")` のキーボードテスト | `within(dishTablist()).getAllByRole("tab")` |

`shows the overall timeline before persistent dish tabs` は見出し比較のまま残してよい。必要なら `getByRole("tablist", { name: "献立の段取りと材料" })` の存在を1行追加。
...
```bash
git add src/features/menu-detail/menu-ingredients-summary.tsx src/features/menu-detail/menu-ingredients-summary.test.tsx src/features/menu-detail/menu-steps.tsx src/features/menu-detail/menu-steps.test.tsx src/features/generation/components/menu-result.test.tsx
...
横幅計測ブロック（おおよそ L402–427）:

```ts
    const timeline = page.getByRole("heading", { name: "全体の段取り" }).locator("..");
    // 親が .cook-timeline-panel であること（DOM ロック）
    ...
    const tabpanel = page.locator(`#${CSS.escape(dishPanelId)}`);
...
同 spec 内で、材料まとめタブを一度開いて横幅を見るアサーションを追加（同一 viewport ループ内、またはループ後に1回）:
...
```ts
  ...
  await expectContainedHorizontally(
    page.getByRole("heading", { name: "全体の段取り" }).locator(".."),
    ingredientsPanel,
  );
  // 段取りに戻して以降の料理タブ操作と干渉しない
  await page.getByRole("tab", { name: "段取り" }).click();
```

（挿入位置は「段取りセクション確認の直後」または横幅ループの前後。料理タブ `.nth(1)` より前なら `getByRole("tab", { name: "材料まとめ" })` は一意。）
