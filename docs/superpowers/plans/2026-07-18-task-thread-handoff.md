...
- producerはGit正本からworktree rootを解決し、全祖先の実directory・非symlinkと作成先directoryのworktree内canonical pathを非追従方式で確認する。
- workspace外操作を拒否できるworkspace-scoped file toolで、検証済みdirectory handle相当へ束縛した `O_CREAT|O_EXCL` 相当の単一排他作成を行う。存在確認と書込を分離せず、安全性またはexclusive createを確認できないsurfaceでは作成せずblockerとする。
...
- handoffは通常ファイル・非symlink・worktree内canonical pathであることを確認する。欠損、malformed、stale、改ざん、正本との不一致、読取中の状態変化はblockerとし、安全な新規発行と再照合成功までTask作業を一切開始しない。
...
Run: `rg -n 'handoff-plan|O_CREAT\|O_EXCL|排他|workspace-scoped|producer|全祖先|canonical path|blocker|Taskの作業を一切開始しない' AGENTS.md`

Expected: 一意名、producer祖先検証、worktree内canonical path、workspace-scoped tool、単一排他作成、既存leafのblocker、consumer再照合、fail-closedなTask開始条件の各規則が表示される。
...
Expected: 終了コード0。一意名、producer祖先検証、worktree内canonical path、workspace-scoped tool、単一排他作成、既存leafのblocker、consumer再照合、fail-closedなTask開始条件の各規則が表示される。
