...
Run: `rg -n '新しいサブエージェントスレッド|handoff-plan|一度だけ|raw diff|progress\.md|glob|自動探索|Taskの作業を一切開始しない|上書き|削除' AGENTS.md`

Expected: 新規スレッド、一意名、write-once、禁止内容、exact path伝達、探索禁止、正本照合、fail-closedなTask開始条件、上書き・削除禁止の各規則が表示される。
...
Expected: 終了コード0。新規スレッド、一意名、write-once、禁止内容、exact path伝達、探索禁止、正本照合、fail-closedなTask開始条件、上書き・削除禁止の各規則が表示される。
