...
- Produces: `docker compose -f compose.tooling.yaml run --rm local-secrets --force`、PG17対応済み `.env`、後続Taskが使用する `vendor-supabase` サービス定義。
...
```bash
docker compose -f compose.tooling.yaml run --rm --entrypoint sh vendor-supabase -c 'git add compose.tooling.yaml scripts/generate-local-secrets.sh scripts/generate-local-secrets.mjs .env.example tests/tooling/project-config.test.mjs tests/tooling/compose.test.mjs && git commit -m "chore: ローカルツールをDocker実行へ統一"'
```
...
```bash
docker compose -f compose.tooling.yaml run --rm --entrypoint sh vendor-supabase -c 'git add tests/tooling/compose.test.mjs compose.yaml Dockerfile.db-test supabase/config.toml supabase/tests/database/001_extensions_and_schemas.test.sql && git commit -m "chore: データベース環境をPostgres 17へ統一"'
```
...
```bash
docker compose -f compose.tooling.yaml run --rm --entrypoint git vendor-supabase diff --exit-code -- src/shared/types/database.generated.ts
```
...
```bash
docker compose -f compose.tooling.yaml run --rm --entrypoint sh vendor-supabase -c 'git add docs/local-development.md && git commit -m "docs: Docker開発環境の手順を追加"'
```
...
```bash
docker compose -f compose.tooling.yaml run --rm --entrypoint git vendor-supabase status --short
docker compose -f compose.tooling.yaml run --rm --entrypoint git vendor-supabase log -5 --oneline
```
