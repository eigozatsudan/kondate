...
**Goal:** このリポジトリで `git push` の毎回承認を維持しながら、`git worktree` の全サブコマンドと `docker compose run` を承認なしで実行できるようにする。

**Architecture:** 既存の `.codex/config.toml` と `git push` の `prompt` ルールは変更しない。`.codex/rules/default.rules` に2つの `allow` ルールを追加し、Codex CLI の execpolicy 検証で許可範囲と対象外コマンドを確認する。
