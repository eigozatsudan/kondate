...
- `resolveGenerationIntegrityContext(admin,userId,command)`がHMAC前に権威あるdraft revisionまたはsource menuを読む。`reserve(command,integrity)`はDBロック下で同じmode/servings/member IDs/source versionを再確認し、不一致なら永続request/quotaを作らない。
...
Run: `docker compose run --rm --no-deps app npx vitest run shared/contracts/generation.test.ts netlify/functions/_shared/generation-command-integrity.test.ts netlify/functions/_shared/generation-integrity-context.test.ts netlify/functions/_shared/generation-repository.test.ts netlify/functions/generation-command-migration.test.ts src/features/generation/model/pending-generation.test.ts src/features/generation/hooks/use-generation-recovery.test.tsx`
...
通常pgTAPへv1/v2 CHECK、未知版拒否、新規v1予約拒否、既存v1 replay、request snapshot immutable、source version変更/削除、遅延v1拒否、quota不変を追加する。`ai_control_and_quota_races.test.sql`は既存shopping race testと同じ専用dblink roleを使い、fixtureをcommitしてから別backend 2 sessionで同じlegacy keyをclaimし、同じv2 keyが返ることを検証する。

- [ ] **Step 4: v2 contractと権威あるintegrity resolverを実装する**
...
`resolveGenerationIntegrityContext`はnew menuで`draftId+draftRevision+owner`、regenerationで`sourceMenuId+owner`と対象dishを取得し、クライアントからmode/servingsを受け取らない。new menuはfrozen候補のmode/servings/member IDs、regenerationはmenuの保存済みmode/servings/member IDs/versionを返す。

- [ ] **Step 5: v2 HMACとrepository予約境界を実装する**

v2 canonical JSONのkey順を次に固定する。
...
kindに存在しない値は`null`で固定し、member IDsと期限切れ確認はsortする。repositoryはresolver値とHMACをreserve RPCへ渡す。RPCはreplay lookupを最初に行い、保存済み行がv1ならv1 HMACだけ、v2ならv2 HMACだけで比較する。replay missのv1は`client_update_required`でrow/quotaを作らない。v2はdraft/sourceをlockしてresolver値と一致したときだけrequestとsnapshotを同一transactionで作る。

- [ ] **Step 6: private snapshotとlegacy claim ledgerをmigrationへ実装する**

`private.generation_regeneration_snapshots`は`request_id` PK/FK cascade、user、kind、source menu ID/version、nullable replace dish、mode、servings、member IDs、created_atを持ち、kind/mode相関CHECKを付ける。UPDATE/INSERT/DELETEをpublic rolesからrevokeする。UPDATEを常に例外にするprivate triggerを付ける。

`private.generation_command_migrations`はuser+legacy keyをPK、v2 key unique、created_at、expires_atを持つ。claim RPCは`auth.uid()`所有者で既存v1 requestを先に返し、なければ`insert ... on conflict ... do update set legacy_idempotency_key = excluded.legacy_idempotency_key returning`相当の単一key読出しを行う。期限は作成時から30日。legacy keyがtombstone済みならreserve RPCはv2 mappingを返してquotaを触らない。
...
- [ ] **Step 7: pending commandの安全なv1→v2変換を実装する**

既存v1 requestがあればv1としてstatus回収する。存在しないnew menuは同じdraft revisionを再読込し、非空targetの旧版契約に限ってhouseholdへ変換する。空target、欠損、revision不一致は現在の回答を維持し対象stepへ戻す。regenerationはmigration済みsource menuのhousehold modeとversionを使う。claim APIが返したv2 keyでv3 storageへの保存が成功した後だけv2 legacy keyを削除する。両storage keyがある場合はv3を正本とし、owner、24時間TTL、request IDの整合が取れたlegacy行だけを削除する。

- [ ] **Step 8: source snapshotのfail-closed動作を実装する**

`regeneration-context.ts`はrequest snapshotを正本としてlive sourceと対象dishをowner/version付きで再取得する。外部送信前の不一致・削除は`source_menu_changed`でfailしattemptを返却する。finalize RPCは再度version/lineageを検査し、送信後の不一致はmenuを作らずattempt消費・success枠非消費で同codeへ終端化する。
...
Run: `docker compose run --rm --no-deps app npx vitest run shared/contracts/generation.test.ts netlify/functions/_shared/generation-command-integrity.test.ts netlify/functions/_shared/generation-integrity-context.test.ts netlify/functions/_shared/generation-repository.test.ts netlify/functions/_shared/regeneration-context.test.ts netlify/functions/generation-command-migration.test.ts netlify/functions/generate-menu.test.ts netlify/functions/generate-dish.test.ts src/features/generation/api/generation-api.test.ts src/features/generation/model/pending-generation.test.ts src/features/generation/hooks/use-generation-recovery.test.tsx src/features/history/hooks/use-regeneration.test.tsx`
...
- Modify: `supabase/migrations/20260711004000_shopping_lists.sql` only through `CREATE OR REPLACE FUNCTION` definitions in the new migration; do not edit the historical file.
...
Run: `docker compose run --rm --no-deps app npx vitest run shared/safety/idea-fingerprint.test.ts shared/safety/validate-generated-menu.test.ts netlify/functions/_shared/generation-context.test.ts netlify/functions/_shared/regeneration-context.test.ts netlify/functions/_shared/generation-prompt.test.ts netlify/functions/_shared/generation-materializer.test.ts netlify/functions/_shared/generation-repository.test.ts netlify/functions/_shared/generation-service.test.ts netlify/functions/_shared/generation-adversarial.integration.test.ts`
...
`private.idea_safety_fingerprint()`は`digest(convert_to('{"assurance":"none","members":[],"mode":"idea"}','UTF8'),'sha256')`をhex化する。finalize RPCはrequest snapshotのmodeで分岐し、ideaでは対象家族0件、family子行0件、保存人数一致、固定snapshot/fingerprint一致、version列nullを検査する。householdでは既存`lock_and_assert_current_safety_fingerprint`をそのまま呼ぶ。両modeでowner、request status、HMAC、source lineage、quotaを維持する。

- [ ] **Step 7: shopping RPCのidea拒否を新migrationで置換する**

両RPCの先頭にあるidempotency replay lookupを移動しない。replay miss後にmenuを`FOR UPDATE`でowner/version付き取得し、直後に次を実行する。
...
export type MenuResultViewModel = {
  targetMode: TargetMode;
  // 既存menu、member、label、pantry aggregateを維持
};
```
...
route層はdraft/autosave/conflict/usage/generationを維持し、表示だけ`PlannerWizard`へ委譲する。各stepは`value`と`onChange`だけを受けSupabaseを呼ばない。audienceでhouseholdを選ぶと利用可能家族1人以上を要求しservingsをnull、ideaを選ぶとmember IDsを空にして人数を毎回明示選択させる。modeを切り替える場合、以前のhousehold IDsまたはidea人数を送信可能状態へ残さない。
...
- [ ] **Step 10: focused verificationを実行する**
...
**Interfaces:**
- Consumes: Task 6の`MenuResultViewModel.targetMode`、mode別result body、Task 4/5のsnapshot/finalize。
- Produces:
...
export type MenuResultModeChangeFields = {
...
- [ ] **Step 6: mode維持再生成を実装する**

clientはsource menu ID、dish ID、reasonだけをv2 wireで送る。mode/servings/member IDsは送らず、Task 4 snapshotが元menuから複製する。`RegenerationSheet`はideaで`child_friendly`をfilterする。serverはsnapshot作成後かつmarkSent前にidea+child_friendlyを拒否する。whole/dish両方でsource mode、servings、lineage versionを維持する。
