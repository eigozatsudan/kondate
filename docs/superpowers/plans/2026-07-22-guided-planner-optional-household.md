...
`lookup_ai_generation_request`はmiss時`{"kind":"miss"}`、hit時`kind`、既存request payload、`requestHmacVersion`、v2だけ`integrityContext`を返す。`reserve_ai_generation`は既存`QuotaRequestRecord` JSONを返し、v2以外の新規予約を拒否する。`claim_generation_command_migration`は`{"legacyIdempotencyKey": uuid, "v2IdempotencyKey": uuid, "replayed": boolean}`だけを返す。
...
claim RPCは`auth.uid()`がnullなら拒否し、ownerの既存v1 requestを先に返す。未claimならtransaction-scoped advisory lockを`user_id+legacy key`へ取得し、再読込後に1つのv2 keyをinsertする。`ON CONFLICT`で既存のv2 keyを変更してはならない。legacy keyがmapping済みならreserve RPCはv2 mappingを返してquotaを触らない。レスポンス作成後に`expires_at`を延長しない。
...
export type GenerationRepository = {
  // 既存のreserve/fail/conflict/statusを維持する。
...
- `GenerationRepository.succeed`と`finalize_ai_generation_success`は上の判別可能unionを唯一の入口にする。ideaでversion文字列や家族対象をサム値に置換しない。
