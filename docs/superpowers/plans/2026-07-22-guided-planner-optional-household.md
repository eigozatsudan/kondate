...
| `src/features/generation/components/generation-status-panel.tsx` | 更新必須と通信失敗を分離した回復表示 |
...
`src/styles.contrast.test.ts`へ設計色の完全一致、本文/補足/主要操作の4.5:1、hover/activeの4.5:1、focus色の存在を追加する。カードの18〜20px角丸、薄いshadow、共通wizard操作の44px以上という既存visual/CSS契約も固定する。`wizard-ui.test.tsx`へ次のassertionを書く。
...
`current-safety-summary.test.tsx`では代表的な既存安全表示が状態を文字と適切な`role`で伝え、色だけへ退行しないことを固定する。
...
```bash
git add src/styles.css src/styles.contrast.test.ts src/shared/ui/wizard src/features/planner/current-safety-summary.tsx src/features/planner/current-safety-summary.test.tsx
...
`onboardingStatuses`へ`skipped`を追加する。`HouseholdOnboardingApi.setProgress`は`OnboardingStatus`を受けるが、画面から送る値は`in_progress|complete`だけとする。家族設定完了操作は`completeMember`成功後に`setProgress("complete")`を呼び、その成功後だけ`/planner`へ遷移する。完全なmemberが既に存在する設定完了操作も同じ順序を使い、どちらかが失敗した場合は現在画面に残って再試行可能なエラーを表示する。`PrivacyNoticePage`から`setOnboardingStatus(...,"complete")`を削除し、同意保存後はsanitized `returnTo`へ遷移する。privacy copyは両mode共通送信内容、householdだけの家族情報、ideaでは家族情報を送らないことを3項目で示す。component testは`completeMember→setProgress→navigate`の順序、各失敗時の非遷移、privacy同意なしでも完了済みprofileが現行guardを通ることを固定する。
...
pgTAPへhousehold/ideaの正常行、4種類の矛盾行、既存menu backfill、空target draftのnull移行、version列のmode別nullabilityを追加する。空targetは少なくとも「他回答が完成済み」「途中回答」「以前は家族を選択していたが保存時点では0件」の3 fixtureを用意する。旧schemaから選択意図を復元できないため、3件とも`target_mode is null`、`servings is null`へ移行し、食事・食材・ジャンル・任意条件・pantry選択・revisionは変更されないことを検証する。さらにTask 3単独のmigration状態で、現行v1 householdの下書き保存→14引数予約→完了を通し、凍結提出と完成献立へ`household`が保存される統合caseを追加する。この回帰caseはTask 4がv1新規予約を置換するまで削除しない。
...
CLI生成migrationでplanner draft、`private.generation_draft_submission_versions`、`public.menus`へ`target_mode`を追加する。`generation_drafts.target_mode`と同表の`servings`は質問途中を表すためnullableとする。`private.generation_draft_submission_versions.target_mode`はbackfill後NOT NULL、同表の`servings`はhouseholdでNULL・ideaで1〜20を表すためnullableとする。`public.menus.target_mode`と実献立人数である既存`menus.servings`はbackfill後もNOT NULLとする。既存draftの非空targetはhousehold、空targetは回答進捗や過去の選択意図に関係なくnullとし、servingsもnullへ揃える。既存凍結提出とmenuはhouseholdへbackfillする。backfillは対象列以外の回答JSON、任意条件、pantry選択、revisionを変更しない。
...
Task 4まで残る現行14引数`public.reserve_ai_generation`も同じmigrationで置換する。このv1経路は`household`だけを受理し、ロック済みdraftの`target_mode='household'`、対象1〜20件、`servings is null`を確認してから、凍結提出の新列へ`household,NULL`を保存する。ideaまたは未選択draftはrequest/quota行を作る前に拒否する。現行`finalize_ai_generation_success`および内部の献立永続化も、v1 requestの凍結提出から`household`を確認し、`menus.target_mode='household'`を明示保存する。signature、HMAC、quota、lock順は変えず、既存のrevoke/grantを再宣言する。このTask 3互換経路はTask 4のv2 migrationが14引数reserveをDROPするまで維持する。
...
Expected: すべてexit 0。既存household fixtureは明示的に`targetMode: "household", servings: null`へ更新され、Task 3だけの状態でv1 household保存・予約・完了が成功する。
...
export class GenerationApiError extends Error {
  ...
  constructor(message: string, code: GenerationFailureCode, retryable: boolean) {
    super(message);
...
export type GenerationMigrationRedirect = {
  status: "migration_redirect";
  legacyIdempotencyKey: string;
  idempotencyKey: string;
};

export type GenerationCommandResponse =
  | GenerationStatusData
  | GenerationMigrationRedirect;
...
- `lookupGenerationRequest(userId,idempotencyKey)`を常に最初に実行し、`既存request → generation_command_migrations tombstone/mapping → true miss`の順で解決する。ledger hitは保存済み版と凍結submission/snapshotからHMAC contextを再構築し、live draft/menuを読まずreplayする。mapping hitの遅延v1 HTTP commandは対応する同一v2 keyへredirectし、予約・quota・attempt・外部AI送信を行わない。true missのv2だけが`resolveGenerationIntegrityContext(admin,userId,command)`で権威あるdraft revisionまたはsource menuを読む。`reserveNew(command,integrity)`はDBロック下で同じmode/servings/member IDs/source versionを再確認し、不一致なら永続request/quotaを作らない。
...
- `GenerationRequestLookup`はHTTP wireではなくrepository内部型とする。mapping hitを公開するwireはHTTP 200 success envelopeの`GenerationMigrationRedirect`だけとし、`{ status: "migration_redirect", legacyIdempotencyKey, idempotencyKey: v2Key }`へ固定する。
- `private.generation_regeneration_snapshots`はrequest 1対1・immutable・cascade 30日保持。`private.generation_command_migrations`は`(user_id,legacy_idempotency_key)`一意でv2 key/tombstoneを、AI ledger 30日、legacy pending TTL、最大processing期限を合わせた期間以上保持する。本実装ではclaim時から31日を下限とし、対応v2 requestが保持中ならmapping cleanup対象にしない。
- legacy migration mappingはownerごとの有効行を最大32件、新規作成を10分間に最大8件へ制限する。same-keyの冪等claimと既存v1 requestの回収は新規作成として数えない。claim transactionは認証user単位のadvisory xact lockで直列化し、bounded expired cleanup後に件数とrateをDBで権威的に検査する。超過時は直接RPCとHTTPの両方で安定code `legacy_migration_limit`を返す。
...
`lookup_ai_generation_request`はownerの既存requestを最優先し、次にmigration mappingを読み、true missだけ`{"kind":"miss"}`を返す。request hitは既存request payload、`requestHmacVersion`、v2だけ`integrityContext`を返し、mapping hitは`migration_redirect`と対応するv2 keyを返す。`reserve_ai_generation`は既存`QuotaRequestRecord` JSONだけを返し、v2以外の新規予約を拒否する。旧14引数overloadはDROP後に再作成せず、既存v1回収と遅延v1 redirectはHTTP/lookup層だけを使う。`claim_generation_command_migration`は`GenerationCommandMigrationResult`と同じ`existing_v1 | claimed_v2`の判別可能JSONを返す。
...
endpoint testsは`commandVersion`なしのtrue missだけを`client_update_required`、既存v1 ledgerをreplay、claim済みtombstoneをHTTP 200の`{ status: "migration_redirect", legacyIdempotencyKey, idempotencyKey: v2Key }`へredirect、v2を通常処理として固定する。`generationResponse`がこの判別可能unionをsuccess envelopeとして返し、新bundleのbrowser API parserが`status`で安全に判別すること、redirect前後でrequest/quota/attempt/provider callが0件のままであることも検証する。新bundleはredirectを受けたら同じv2 keyのv3 pendingを保存してから通常のv2 commandを送る。配信済み旧bundleのparser fixtureは未知の`migration_redirect`をrejectし、既存recoveryのgeneric offline表示になり得ることを現実の契約とする。その場合もサーバー側で新規v1予約、quota、attempt、provider call、mapping変更が0件であるfail-closed性だけを保証し、旧bundleが再読込文言やCTAを表示するとは要求しない。

browser/model/hook/component testは`GenerationApiError.code`を保持し、`generation-machine.ts`へ`update_required` phase/event/reducerを追加して、`client_update_required`を通信切断やretryable failureとして扱わず、非再送の終端状態、「画面を再読み込みしてください」という文言、再読込CTAへ写すことを固定する。`update_required`中の`online`/`network_error`/通常retryではofflineのように`checking`へ復帰せず、明示的なreloadだけを回復操作にする。型付き`migration_redirect`はこのerror eventとは別caseとしてv3 pending保存へ進むことも固定する。
...
Run: `docker compose run --rm --no-deps app npx vitest run shared/contracts/generation.test.ts netlify/functions/_shared/generation-command-integrity.test.ts netlify/functions/_shared/generation-integrity-context.test.ts netlify/functions/_shared/generation-repository.test.ts netlify/functions/_shared/generation-service.test.ts netlify/functions/_shared/generation-adversarial.integration.test.ts netlify/functions/generation-command-migration.test.ts src/features/generation/api/generation-api.test.ts src/features/generation/model/pending-generation.test.ts src/features/generation/model/generation-machine.test.ts src/features/generation/hooks/use-generation-recovery.test.tsx src/features/generation/components/generation-status-panel.test.tsx`
...
migration適用前から`processing`だったことを再現する、`request_hmac_version='generation-command.v1'`かつsnapshotなしの`regenerate_menu`と`regenerate_dish` fixtureを作る。それぞれで旧signatureの成功finalizeがhousehold献立とlineageを保存しterminal化できること、失敗finalizeもsnapshotを要求せずterminal化できること、quota/success/attemptの返却・消費契約と後続lookup/replay回収が整合することをpgTAPと`generation-adversarial.integration.test.ts`で固定する。予約時のsource versionはv1台帳から復元できないためbackfillせず、このfixturesに新snapshot/source-version検査を要求しない。

通常pgTAPへv1/v2 CHECK、未知版拒否、新規v1予約拒否、既存v1 replay、request snapshot immutable、source version変更/削除、遅延v1 mapping redirect、quota不変を追加する。旧14引数`reserve_ai_generation`の`to_regprocedure(...) is null`と、新15引数signatureの`to_regprocedure(...) is not null`を別assertionで検証し、旧signature呼出し失敗前後でrequest/usage行数不変も検証する。snapshotの別owner request、21人、NULL要素、重複IDを直接INSERTとRPCの両方で拒否する。legacy mapping制限はownerの有効31件から32件目が成功し33件目が`legacy_migration_limit`、10分内8件目が成功し9件目が同code、same-key再claimと既存v1回収は作成count不変、別ownerは独立、と直接RPC/endpoint testで固定する。`ai_control_and_quota_races.test.sql`は既存shopping race testと同じ専用dblink roleを使い、fixtureをcommitしてから別backend 2 sessionで同じlegacy keyをclaimする。既存v1が先に作られたraceは両sessionが`existing_v1`と同一requestを返し、未作成raceは両sessionが`claimed_v2`と同一v2 keyを返し、どちらもrequest/quotaを二重作成しない。別keyの並行直接RPCも認証user単位lockで直列化され、合計件数/rate上限を越えるinsertがなく一方が安定codeで終端する。claim完了後に遅延v1 HTTP commandを送ったcaseは両sessionで同じ`migration_redirect`のv2 keyへ収束し、副作用がないことを検証する。
...
`runGeneration`はparse済みcommandからidempotency keyを取り、最初のrepository callを必ず`lookup`にする。hit時はv1ならincoming legacy command、v2ならlookupが返した凍結integrityでHMACを再計算し、`replayExisting`へ渡す。`migration_redirect`は対応するv2 keyをHTTP responseへ返して終了し、resolver、reserve、quota、attempt、providerを呼ばない。元draft/menu/dish、privacy、pantry、householdをhit/redirect前に読まない。保存HMAC不一致は`idempotency_payload_mismatch`、一致は保存済みstatusを返す。

`lookup`と`replayExisting`の間に行が消えた場合は1回だけmiss処理へ戻さず`internal_error`でfail-closedする。30日retention中のrowが通常実行で消えない既存契約を回帰テストする。

lookup hitが`requestHmacVersion: "generation-command.v1"`かつ`processing`の`regenerate_menu | regenerate_dish`なら、Task 3時点のlegacy household context/finalize分岐へ入れる。この分岐は新しいregeneration snapshotを検索せず、v1が予約時に保存したsource menu/dishと従来の家族安全コンテキストで続行する。新規v1予約の禁止は維持し、このreaderとlegacy finalize分岐は既存v1 requestが保持期間と最長処理期限を超えて存在しなくなるまで削除しない。

- [ ] **Step 6: replay miss専用integrity resolverを実装する**

`resolveGenerationIntegrityContext`はnew menuで`draftId+draftRevision+owner`、regenerationで`sourceMenuId+owner`と対象dishを取得し、クライアントからmode/servingsを受け取らない。new menuはfrozen候補のmode/servings/member IDs、regenerationはmenuの保存済みmode/servings/member IDs/versionを返す。new menuとregenerationの両方をmode別discriminated unionへ絞り、householdはNULL要素・重複なしの非空tupleかつruntime 1〜20、ideaは空tupleとする。矛盾は型生成元のZodとruntime resolverの両方で拒否する。resolverは`lookup.kind === "miss"`かつv2の場合だけ呼ぶ。true missのv1だけresolverを呼ばず`client_update_required`を返す。
...
`private.ai_generation_requests`へ`(id,user_id)`のUNIQUE制約を追加した上で、次の列・制約をmigrationへ定義する。snapshotの`(request_id,user_id)`はその複合keyへ`ON DELETE CASCADE`で参照し、request ownerとsnapshot ownerを構造的に一致させる。`private.is_valid_generation_target_member_ids(uuid[],text)`相当のimmutable helperを作り、一次元、NULL要素なし、重複なし、household 1〜20件、idea 0件をCHECKから強制する。reserve RPCでもsort済み配列を保存する。`kind='regenerate_dish'`の時だけ`replace_dish_id`を必須にする。
...
両tableはschema/table権限を`public,anon,authenticated`からrevokeする。配列検査helperも`public,anon,authenticated,service_role`から`EXECUTE`をrevokeし、CHECKからだけ利用する。snapshotはUPDATEを常に例外にするprivate triggerを付け、作成はreserve RPC、削除はrequest cascadeとcleanupだけに限定する。

- [ ] **Step 9: legacy claim ledger、RPC、bounded cleanupを実装する**

claim RPCは`auth.uid()`がnullなら拒否し、次の認証user単位transaction-scoped advisory lockを取得する。keyごとのlockではなくowner全体を直列化し、同一ownerの別key並行claimも上限判定をすり抜けない。

```sql
perform pg_advisory_xact_lock(
  hashtextextended('generation-command-migration-owner:' || v_user_id::text, 0)
);
```

hash衝突は無関係なclaimを直列化するだけで正しさを損なわない。lock後にv1 requestとmappingを再読込し、既存v1は`existing_v1`、same-key mappingは同じ`claimed_v2`を返して新規作成countに含めない。真の新規作成だけ、対応requestを保護するowner内最大100件のbounded expired cleanup後に、`expires_at > clock_timestamp()`の有効mappingが32件未満、`created_at >= clock_timestamp() - interval '10 minutes'`の新規mappingが8件未満であることを同じtransaction内で検査する。どちらかを超える場合はinsertせず、直接RPCの安定codeを`legacy_migration_limit`へ固定する。HTTP endpointもこれを安定したrate-limit response code `legacy_migration_limit`へ写す。通過時だけ新しいv2 keyを`insert ... on conflict (user_id, legacy_idempotency_key) do nothing`で追加する。主キーを競合時の最終防壁とし、insert後は同じ主キーで必ず再selectして`claimed_v2`を返す。`ON CONFLICT DO UPDATE`で既存のv2 keyや期限を変更してはならない。mapping判定はHTTP/lookup/claim層へ集約する。新15引数DB reserve RPC自体はmigration redirectを返さず、v2の通常予約と同一key replayだけを扱う。レスポンス作成後に`expires_at`を延長しない。

期限切れmappingは`private.cleanup_generation_command_migrations(p_before timestamptz,p_limit integer)`で`created_at,user_id,legacy_idempotency_key`順に最大250件だけ削除し、service role以外のexecuteをrevokeする。claim時のowner cleanupも自分の期限切れ行を最大100件に制限する。ただし、どちらのcleanupも対応するv2 requestがAI ledgerの保持期間内ならmappingを削除しない。claimから31日、pending TTL直前の予約、最大processing期限直前のterminal化、対応request保持終了の各境界で、requestが残る間はredirectでき、requestとmappingがともに保持終了した後だけbounded cleanupされることをpgTAP/Vitestで固定する。

- [ ] **Step 10: pending commandの安全なv1→v2変換を実装する**

ここで実装する型付き`migration_redirect`/`client_update_required`、非retryの`update_required`、再読込文言とCTAは新bundleの契約である。配信済み旧bundleは未知successをrejectしgeneric offlineを表示し得るため、旧bundleに再読込案内を保証しない。旧bundleについては、サーバーが新規v1予約、quota、attempt、provider call、mapping変更を行わないfail-closed性を保証する。

claim結果が`existing_v1`なら内包されたrequest statusをv1として回収し、変換もv3書込みも行わない。`claimed_v2`なら返されたv2 keyを使って変換する。存在しないnew menuは同じdraft revisionを再読込し、非空targetの旧版契約に限ってhouseholdへ変換する。空target、欠損、revision不一致は現在の回答を維持し対象stepへ戻す。regenerationはmigration済みsource menuのhousehold modeとversionを使う。v3 storageへの保存が成功し、owner、TTL、request IDとmappingの整合を再確認した後だけv2 legacy keyを削除する。変換不能なlegacyは回答復元に必要な範囲を保持し、送信可能状態から外す。両storage keyがある場合はv3を正本とし、owner、現行`PENDING_GENERATION_TTL_MS`の30分、request IDの整合が取れたlegacy行だけを削除する。

`generationResponse`は`GenerationCommandResponse`をHTTP 200 success envelopeとして返す。`generation-api.ts`はsuccess bodyも`status`でparseし、`migration_redirect`なら`legacyIdempotencyKey`と`idempotencyKey`を保持してrecoveryへ渡し、HTTP error bodyは`GenerationApiError`へparseしてcodeを保持する。recoveryはredirect時に返却された同じv2 keyでv3 pendingを保存できたことを確認してから、そのv2 commandを通常送信する。保存失敗時や未知success statusでは新しいkeyを生成・予約せずfail-closedして再読込を促し、legacy mappingを変更しない。`generation-machine.ts`は`client_update_required`専用eventから`{ phase: "update_required", effect: "none" }`へ遷移し、このphaseの`online`、`network_error`、`recover`、`submit`を無視するterminal reducer契約を持つ。`use-generation-recovery`は同codeを自動retry、offline待機、同じlegacy commandの再送から除外してこのeventをdispatchする。型付き`migration_redirect`は別caseとして同じv2 keyのpending保存へ進む。`GenerationStatusPanel`は平易な再読込案内と`window.location.reload()`へ結ぶ「画面を再読み込み」操作を表示する。再読込操作前にlegacy pendingを無条件削除せず、既存request/mappingを回収できる行は保持し、true missで変換不能と確定した行だけ送信不能化する。endpoint→success/error parser→typed redirect/client-update-required case→pending保存→machine→hook→表示までを一続きで検証する。

- [ ] **Step 11: source snapshotのfail-closed動作を実装する**

`regeneration-context.ts`は最初にrequest HMAC versionを判定する。v2だけはrequest snapshotを正本とし、`generation-command.v1`かつ既存`processing`の`regenerate_menu | regenerate_dish`はlegacy household分岐へ入る。legacy分岐はTask 3時点の家族安全、household保存、lineage、quota/attempt契約を維持して旧signatureの成功・失敗finalizeを実行する。v1は予約時versionを復元できないためsnapshotをbackfillせず、新snapshot/source-version検査を要求しない。新規v1予約は禁止し続け、v1 reader/legacy finalizeは既存v1 requestがなくなるまで保持する。
...
Run: `docker compose run --rm --no-deps app npx vitest run shared/contracts/generation.test.ts netlify/functions/_shared/generation-command-integrity.test.ts netlify/functions/_shared/generation-integrity-context.test.ts netlify/functions/_shared/generation-repository.test.ts netlify/functions/_shared/generation-service.test.ts netlify/functions/_shared/generation-adversarial.integration.test.ts netlify/functions/_shared/regeneration-context.test.ts netlify/functions/generation-command-migration.test.ts netlify/functions/generate-menu.test.ts netlify/functions/generate-dish.test.ts src/features/generation/api/generation-api.test.ts src/features/generation/model/pending-generation.test.ts src/features/generation/model/generation-machine.test.ts src/features/generation/hooks/use-generation-recovery.test.tsx src/features/generation/components/generation-status-panel.test.tsx src/features/history/hooks/use-regeneration.test.tsx`
...
```bash
git add shared/contracts/generation.ts shared/contracts/generation.test.ts netlify/functions src/features/generation/model/generation-machine.ts src/features/generation/model/generation-machine.test.ts src/features/generation src/features/history/hooks/use-regeneration.ts src/features/history/hooks/use-regeneration.test.tsx src/shared/types/database.generated.ts supabase/migrations supabase/tests/database/ai_control_and_quota.test.sql supabase/tests/database/ai_control_and_quota_races.test.sql supabase/tests/database/history_regeneration.test.sql netlify/functions/_shared/generation-adversarial.integration.test.ts
...
**Interfaces:**
- Consumes: Task 4のv2 integrity、request snapshot、source fail-closed契約と、snapshotなしの既存processing v1再生成だけを完了させるlegacy household分岐。
- Produces:
...
`regeneration-adapter.test.ts`はidea snapshotからの再生成がcurrent household safetyと`buildStoredGenerationContext`を呼ばず、専用idea contextを返すこと、household branchは既存builderと家族安全検査を維持することを先に固定する。

さらに`request_hmac_version='generation-command.v1'`かつsnapshotなしの既存`processing` whole/dish fixtureは、adapterがsnapshot lookupより先にlegacy household分岐を選び、Task 3の家族安全contextを構築できることを固定する。v2 fixtureは引き続snapshot必須とする。
...
ただし再生成adapterはrequest snapshotを読む前に`request_hmac_version`を判定する。`generation-command.v1`かつ既存`processing`の両再生kindはTask 4のlegacy household contextを使い、新snapshot/source-version検査を要求しない。v2のみsnapshot modeでidea/householdに分岐する。
...
`public.finalize_ai_generation_success`はrequestをlockした直後に`request_hmac_version`を判定する。snapshotなしの既存processing v1 whole/dishはTask 3時点のlegacy household finalizeを先に実行し、household保存、従来lineage、家族安全、quota/attemptを維持する。v2だけが以下のsnapshot mode分岐とsource-version再確認へ進む。同じ版判定を失敗finalizeにも適用し、v1をsnapshot欠落で回収不能にしない。
...
menu result queryへ`target_mode`と`preference_snapshot`を追加する。`preference_snapshot.submission`を`plannerSubmissionSchema.safeParse`し、成功時だけ`sourceSubmission`へ設定する。`MenuResultPage`はaggregate読込後にhousehold bodyまたはidea bodyへコンポーネント境界で分岐し、household bodyだけが`useMenuRevalidation`、active-list/reconcilable query、shopping pending replay hookをmountする。hookのconditional callではなくmode別child componentで分離する。`MenuResultViewModel.targetMode`を唯一のUI判定元とし、household bodyだけが買い物作成button、Create sheet、Reconcile sheetをrenderする。`fetchReconcilableMenuSource`のmenu queryにも`target_mode='household'`を加えるが、これはTask 5のHTTP/DB拒否に対する防御層とする。idea bodyは`InlineNotice`で「家族条件を使用していません」「年齢・アレルギーへの適合は確認されていません」を常時表示し、`MenuResult`へ`mode="idea"`とactionsなしを渡す。`MenuResult`はideaでadaptation、label confirmation、family safety summaryをrenderせず、4つのshopping endpointを呼ばない。

- [ ] **Step 12: 公開時点のhistory read-only境界を実装する**

history一覧selectへ`target_mode`を追加し、group modelへ`targetMode`を保持する。cardは「アイデア」「家族に合わせた献立」の文字badgeでmodeを示す。詳細はmenu aggregateを取得して権威ある`targetMode`を判定した後にmode別child componentへ分岐する。household childは現行family revalidationとshopping経路を維持する。idea childは常時noticeと献立本文だけを表示し、保存済みsnapshotを家族安全表示として解釈しない。family revalidation、shopping query/hook/pending replay、採用、お気に入り、冷蔵庫反映、whole/dish再生成のhook・component・4 endpointはmountしない。Task 7はこの境界を維持したまま許可操作だけを有効化する。

- [ ] **Step 13: 既存E2E導線を新しいrouteへ更新する**

auth fixtureはログイン後の`/welcome`または`/planner`をstatus別に待つ。onboarding、auth recovery、OAuthの旧`/onboarding`強制期待を新root分岐へ変更する。generation E2Eへ「家族設定を省略→4質問→idea人数→privacy→review→生成→notice付き本文」を追加する。Task 6ではidea操作buttonが存在せず、shoppingのcreate/preview/reconcile/revalidate requestが0件、`sessionStorage`に`kondate:shopping:` prefixのkeyが0件であることも確認する。人数の範囲エラーではfield-local descriptionと最初のinvalid field focusを検証する。
...
```bash
git add src/app src/features/auth src/features/welcome src/features/planner src/features/privacy src/features/generation src/features/shopping/api src/features/shopping/hooks src/features/history/api/history-api.ts src/features/history/model/group-history.ts src/features/history/model/group-history.test.ts src/features/history/components/history-card.tsx src/features/history/pages/history-page.tsx src/features/history/pages/history-page.test.tsx src/features/history/pages/history-detail-page.tsx src/features/history/pages/history-detail-page.test.tsx shared/contracts/menu-result.ts e2e/fixtures/auth.ts e2e/specs/onboarding.spec.ts e2e/specs/auth-recovery.spec.ts e2e/specs/oauth-mock.spec.ts e2e/specs/generation-recovery-results.spec.ts
...
同じlegacy keyを2 sessionでclaimし、v2 keyが1つ、request成功が1つ、quota/attemptが1系列であることをpgTAP/race harnessで確認する。別key並行claimでもowner単位lockにより有効32件・10分8件の上限を越えず、33件目/9件目が直接RPCとHTTPで`legacy_migration_limit`、same-key/既存v1はcount不変、別ownerは独立であることを確認する。遅延legacy HTTP commandはledger-first lookupの既存request→mapping順で同じv2 keyへredirectし、旧reserve RPC、quota、attempt、providerを呼ばない。true missだけ`client_update_required`となり、browserが自動再送せず更新必須表示へ到達する。whole/dish再生成は予約後のsource変更・削除を外部送信前と送信後に分け、`source_menu_changed`、attempt返却/消費、success非消費、menu 0件を確認する。
