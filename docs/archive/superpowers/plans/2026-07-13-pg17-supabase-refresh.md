...
- 過去の `docs/archive/superpowers/plans/` と `docs/archive/superpowers/specs/` にあるPG15表記は履歴として変更しない。
...
- Modify: `compose.yaml` — `migrate` をPG17構成へ合わせ、DB型生成が公式Metaサービスへ到達できる依存関係を維持する。
...
finish() {
  ...
  if [ "$operation_completed" != true ]; then
    if [ "$target_installed" = true ]; then
      if rm -rf "$target"; then target_installed=false; else preserve_staging=true; fi
    fi
    if [ "$version_installed" = true ]; then
      if rm -f "$version_file"; then version_installed=false; else preserve_staging=true; fi
    fi
    if [ "$target_backed_up" = true ] && [ "$target_installed" = false ]; then
      if mv "$backup_target" "$target"; then target_backed_up=false; else preserve_staging=true; fi
    fi
    if [ "$version_backed_up" = true ] && [ "$version_installed" = false ]; then
      if mv "$backup_version" "$version_file"; then version_backed_up=false; else preserve_staging=true; fi
    fi
...
- Produces: 同一タグの `db`・`migrate`・`db-test`、公式Meta endpointによる型生成契約、PG17を強制する静的テストとpgTAP。
...
Expected: 静的テストは `migrate`、`db-test`、`major_version`、Meta型生成契約の不一致でFAIL。DBテストは切り替え前のPG15に対するversion assertionでFAIL。
...
RUN apk add --no-cache perl perl-app-cpanminus perl-module-build \
  && archive=/tmp/TAP-Parser-SourceHandler-pgTAP-3.37.tar.gz \
  && url=https://cpan.metacpan.org/authors/id/D/DW/DWHEELER/TAP-Parser-SourceHandler-pgTAP-3.37.tar.gz \
  && wget -qO "$archive" "$url" \
  && printf '%s  %s\n' 6e928581442a1e687131f7b5d6f4ff44b7f8dcdf798d2d076bdcd07d8b7a597d "$archive" | sha256sum -c \
  && cpanm --notest "$archive" \
  && rm -f "$archive"
```

上流タグが設計時点から進んでいる場合は、両方へTask 2の公式完全タグを同じ値で記載する。

- [ ] **Step 5: Supabase CLI設定とMeta型生成をPG17へ合わせる**
...
`scripts/generate-database-types.sh` は `http://meta:8080/generators/typescript` に `public,private` schemaを指定して問い合わせる。Supabase CLIによる入れ子のPodman起動は行わない。HTTP成功と生成されたTypeScript契約を検証し、生成先と同じディレクトリに作成した一時ファイルを検証後に原子的にrenameする。`compose.yaml` とローカルシークレット生成から未使用の `LOCAL_DB_URL` を削除する。
...
Expected: ComposeとNodeテストPASS、固定URLとSHA-256を検証してPG17ベースの `db-test` イメージbuild成功。`apk`、checksum検証、またはpgTAP Perl runnerの導入に失敗した場合は、実装を進めず `superpowers:systematic-debugging` で原因を確認する。
...
Expected: pgTAP 83件がすべてPASS（既存82件＋PG17 assertion）。Meta方式の型生成が成功し、検証済み出力だけが原子的に配置される。
