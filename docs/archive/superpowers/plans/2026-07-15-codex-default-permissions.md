...
### Task 3: Dockerコマンドとホスト側検査の分離実行

**Files:**
- Modify: `AGENTS.md`
- Modify: `docs/archive/superpowers/specs/2026-07-15-codex-default-permissions-design.md`
- Modify: `docs/archive/superpowers/plans/2026-07-15-codex-default-permissions.md`

**Interfaces:**
- Consumes: 既存の `docker compose run` の `allow` ルール、ワークスペース内で実行する `git diff --check`
- Produces: Dockerコマンドとホスト側コマンドを独立したツール呼び出しで実行するリポジトリ規約

- [x] **Step 1: 複合コマンドの権限判定を確認する**

Run:

```bash
codex execpolicy check --pretty --rules .codex/rules/default.rules -- docker compose run --rm --no-deps app npx prettier --check src/features/planner/planner-page.tsx
codex execpolicy check --pretty --rules .codex/rules/default.rules -- git diff --check
codex execpolicy check --pretty --rules .codex/rules/default.rules -- bash -lc 'docker compose run --rm --no-deps app npx prettier --check src/features/planner/planner-page.tsx && git diff --check'
```

Expected: Dockerコマンド単体は `"allow"`、`git diff --check` 単体とシェルラッパー全体は追加ルールに一致しない。

- [x] **Step 2: 分離実行の規約と設計根拠を追加する**

`AGENTS.md` の「ビルド、テスト、開発コマンド」節へ次を追加する。

```markdown
権限確認を避けるため、Docker コマンドと `git diff --check` などのホスト側コマンドを `&&` などで結合せず、それぞれ独立したツール呼び出しとして実行すること。
```

設計書へ、シェルラッパーが保守的に判定されること、`git diff` や `bash -lc` の包括的な許可を追加しないことを記録する。
...
```bash
git diff -- AGENTS.md docs/archive/superpowers/specs/2026-07-15-codex-default-permissions-design.md docs/archive/superpowers/plans/2026-07-15-codex-default-permissions.md
```

Expected: Dockerコマンドとホスト側コマンドを分離する規約、設計根拠、実装計画だけが表示される。

- [x] **Step 4: 文書変更をコミットする**

```bash
git add AGENTS.md docs/archive/superpowers/specs/2026-07-15-codex-default-permissions-design.md docs/archive/superpowers/plans/2026-07-15-codex-default-permissions.md
git commit -m "docs: 権限確認を避ける実行規約を追加"
```

Expected: 3つの文書だけを含む新しいコミットが作成される。
