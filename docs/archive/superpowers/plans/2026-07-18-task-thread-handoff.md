...
- このplanのTask baseは実行開始commit `285c6c0` とする。Verifierは検証開始時のHEADを `TASK_HEAD` として記録し、終了時HEADとの一致をPASS条件とする。
...
- consumerは同じ非並行の脅威モデルの下でworktree rootからhandoff leafの親までの全祖先を静的検査し、symlinkまたは非directoryなら内容を読まずblockerとしてTask開始を停止する。その後にleafの通常ファイル・非symlink・worktree内canonical pathを確認し、exact pathのfileだけを読む。欠損、malformed、stale、改ざん、正本との不一致、予期しない状態変化はblockerとし、安全な新規発行と再照合成功までTask作業を一切開始しない。
...
- Reference: `docs/archive/superpowers/specs/2026-07-18-task-thread-handoff-design.md`
...
Run: `rg -n 'handoff-plan|脅威モデル外|single-writer|静的|workspace-scoped file creation|producer|全祖先|内容を読まず|canonical path|blocker|Taskの作業を一切開始しない' AGENTS.md`

Expected: 一意名、非並行の信頼境界、producerの静的検査と通常surfaceでの作成、既存leafのblocker、consumerの全祖先・leafに対する静的fail-closed検査が表示される。
...
Expected: 終了コード0。出力された完全なSHAを `TASK_HEAD` として記録する。

Run: `git diff --check 285c6c0..HEAD`

Expected: 終了コード0、出力なし。Task base `285c6c0` から記録した `TASK_HEAD` までのTask実装と後続fixを検証対象に含める。
...
Expected: 終了コード0。一意名、非並行の信頼境界、producerの静的検査と通常surfaceでの作成、既存leafのblocker、consumerの全祖先・leafに対する静的fail-closed検査が表示される。
...
Expected: 終了コード0。出力が検証開始時に記録した `TASK_HEAD` と完全一致する。一致したresolved SHAをVerifier reportへ記録する。不一致なら検証中のHEAD変化をblockerとして報告し、PASSにしない。
