...
| 項目 | 値 |
|------|-----|
| 文書 | `docs/archive/superpowers/specs/2026-07-30-free-landing-page-design.md` |
| 日付 | 2026-07-30 |
| 状態 | **Approved for implementation**（実装計画: `docs/archive/superpowers/plans/2026-07-30-free-landing-page.md`） |
...
UX 方針（2026-07-30 改善）: **文章を主・画像は補助**。ヒーローは帯状、カードは小さなサムネ + 本文。できることはフロー 3 ステップ + 機能カード本文と箇条書きで説明する。

| キー（実装定数名は英語 export 可） | 方向 | 禁止・注意 |
|----------------------------------|------|------------|
| ブランド | こんだて日和 | h1 にしない（L17） |
| h1 | 今日の献立、家族に合わせて。 | ページ唯一の h1 |
| リード | アプリの説明（家族・予算時間・食材・一食・無料）を 2 段落まで | 「AI が完璧」「安全保証」 |
| 使い方 | 「はじめての使い方」3 ステップ | 枠数字・Plus なし |
| 機能節 | 「無料でできること」+ 3 カード（見出し・本文・3 点箇条書き） | L5 の 3 主題を維持 |
| 主 CTA | 無料ではじめる | Plus / 課金語 |
| 副リンク | ログイン | |
| 締め | 無料・あとから家族登録可のトーン | Plus 誘導 |

正本の一字句は `FREE_LP_*` 定数（`free-landing-page.tsx`）と unit。**L5–L8・L17 と「Plus 文字列ゼロ」は破らない**。
...
| 実装計画 | **`docs/archive/superpowers/plans/2026-07-30-free-landing-page.md`** |
