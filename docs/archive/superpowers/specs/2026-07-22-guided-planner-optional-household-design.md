...
- welcome、planner wizard、mode-awareな結果・履歴child surfaceに限定したデザイントークンと共通UI部品
- 現行家族モードの安全・所有者・RLS・HMAC・quota・復帰契約の回帰保護
...
対象外画面の抜本変更は、本設計の基盤完成後に独立した設計・実装計画として扱う。対象画面用のリネン＆テラコッタtokenと共通style overrideは専用の `.guided-planner-theme` 配下へ限定し、global `:root`、`body`、共通button/field、全 `.app-section` の既存外観を置換しない。データ契約変更の影響を受ける現行家族モードには回帰テストを行う。

本機能は本番環境その他の永続利用環境へ未デプロイであり、保存済み利用者データや配信済みブラウザコードとのリリース間後方互換は対象外とする。マイグレーションとテストはclean resetを正本とし、過去版の行・端末コマンド・処理中リクエストを移行または回収する仕組みを追加しない。
...
この8色をcore paletteとする。次の色はcore paletteやglobal tokenを置き換える追加色ではなく、`.guided-planner-theme`内で既存の意味を失わないためのscoped functional tokenとして使う。

| 機能上の役割 | 値 |
| --- | --- |
| 境界線 | `#d8c9bc` |
| 冷蔵庫状態 | `#416b5a` |
| danger/error文字 | `#9f342c` |
...
- 本文 `#423a32` / 選択面 `#f4e6df`: 9.15:1
- 補足 `#6b5e52` / 選択面 `#f4e6df`: 5.15:1
- 本文 `#423a32` / 注意面 `#f8ece7`: 9.64:1
- 補足 `#6b5e52` / 注意面 `#f8ece7`: 5.42:1
- danger/error文字 `#9f342c` / 注意面 `#f8ece7`: 6.04:1
- 冷蔵庫状態 `#416b5a` / カード `#fffdf8`: 5.94:1
...
`RootEntryPage`は認証済みuser IDを用い、`householdKeys.profile`をquery key、`getProfile`をquery functionとしてprofile rowを取得する。pending中は進行状況を表示する。query errorまたはprofile row欠損は`not_started`と推測せず、再試行可能な`role="alert"`を表示してredirectしない。取得成功時だけstatusを判定し、`not_started|in_progress`は`/welcome`、`complete|skipped`は`/planner`へ`replace`で遷移する。

`skipped`かつ利用可能な家族0人でも、`/pantry`、`/history`、`/shopping`、`/settings`、`/emergency-menus`の5 routeは直接到達可能とする。いずれもonboardingへ戻さず、例外なく理解可能なempty stateを表示し、利用可能な家族が現れるまでは家族安全query/revalidation requestを開始しない。`/emergency-menus`は既存のunguarded reachabilityを維持し、下書きなしとidea下書きのどちらも家族不在を説明するempty stateへ安全に終端する。
...
- 家族モードの現行product behaviorと家族データの意味は変更しない。
...
生成リクエストHMACには `target_mode` とアイデアモードの `servings` を含める。空の対象家族から `idea` を推測せず、質問途中の下書きはモードと人数を未選択状態として保存する。
...
- 完成献立へNOT NULLの `target_mode` と `household | idea` のCHECKを追加する。
...
- clean resetで新しい列と制約を直接成立させ、旧行を変換するデータ移行処理は追加しない。

保存schemaを追加するTaskだけを適用した中間状態でも、現行household生成を壊してはならない。同じmigrationで`save_generation_draft`をmode/servings対応signatureへ置換し、次Taskが生成コマンドをv2へ原子的に置換するまで、現行household予約・完了を新列へ追随させる。これはTask 3→4間の開発時境界だけをgreenに保つ一時ブリッジであり、リリース間互換、過去版reader、移行台帳、redirectを作る理由にはしない。Task単独でhouseholdの保存、予約、完了を通す。

質問途中で `targetMode` と `servings` がnullの下書きを開く場合、食事、食材、ジャンル、追加条件、冷蔵庫選択は保持し、ウィザードは最初の未完了項目である対象ステップから再開する。
...
生成コマンドとHMAC版は `generation-command.v2` だけを実装し、canonical payloadへ `targetMode`、`servings`、sort済み `targetMemberIds`、再生成時の `sourceMenuVersion` を含める。HTTP wire commandは全kindでトップレベルに `commandVersion: "generation-command.v2"` を必須とする。

新規献立のwire requestは下書きIDとrevisionを送り、サーバーが所有者確認済みの凍結下書きから `targetMode`、`servings`、対象家族を解決する。献立全体・一品の再生成は、サーバーが所有者確認済みの元献立から `targetMode` と実体化済みの `menus.servings`、対象家族、source versionを解決し、request-bound snapshotへ複製してHMACへ含める。再生成のクライアント入力でモード、人数、対象家族、source versionを上書きできない。

予約処理は常に生成リクエスト台帳を最初に検索する。同じv2 idempotency keyのhitは保存済みの凍結提出またはrequest snapshotからHMAC contextを再構築し、live draft/menuを再解釈せず同じ状態を返す。missだけが権威あるdraft/sourceを解決し、DB lock下で同じ値を再確認してrequest、snapshot、quota reservationを同一transactionで作る。HMAC不一致は `idempotency_payload_mismatch` とし、予約枠や外部AI試行枠を二重消費しない。

端末pending保存も `commandVersion` を含むv2 schemaだけを使用する。owner、TTL、request IDの既存検査、通信切断後の同一request回収、保存成功前に送信しない順序を維持する。本番その他の永続環境へ未デプロイのため、過去版reader、保存key変換、移行endpoint、mapping/tombstone、旧形式の処理中request回収は実装しない。
...
再生成のcontext構築とfinalizeはrequest-bound snapshotを必須とし、保存済みmode、人数、対象家族、source versionを正本にする。家族モードではTask 3までの家族安全とlineageを維持した上でsource-versionを再検証する。
...
完成献立のメンバー配列はアイデアモードに限り空を許可する。結果コンポーネントは家族別取り分け、家族別注意、ラベル確認の領域自体を表示しない。履歴など完成献立の利用箇所は、空の対象家族を欠損データと誤判定しないようmode-awareにする。
...
- モード未選択の下書きは回答を保持して対象ステップから再開する。エラーとして破棄せず、アイデアモードへも自動変換しない。
...
- 下書きの未選択状態
- 対象未選択の下書きで回答を保持し、対象ステップから再開すること
- 各質問の選択、戻る、進む、回答保持
- 家族未登録時のアイデアモードと登録導線
- 家族変更時に自動降格しないこと
- AI情報説明から確認画面へ戻ること
...
- v2 pending commandがowner、TTL、request IDを検査し、通信切断後も同一idempotency keyのrequestを回収すること
...
- 保存schema Task単独でTask 3→4の開発時ブリッジとなる現行household下書き保存、予約、完了が通ること
- `menus` のhousehold行はアレルゲン版・年齢別ルール版が必須、idea行は両方NULLで、servings・snapshot・fingerprintは両モード必須であること
- `request_hmac_version` が `generation-command.v2` 以外を拒否すること
- v2予約内容、HMAC、再送時のモード一致
...
- 同じv2 idempotency keyのreplayがrequest、quota、attemptを二重作成・二重消費しないこと
- 同一ownerにprocessing v2があるとき、別keyのnew/whole/dish予約はすべて安定した`generation_in_progress`となり、request、quota、attempt、snapshotが増えないこと
...
- アイデアモードで明示的に人数Nを指定し、結果に`N人分`と表示され、保存されたmenuの`servings`もNと一致する経路（1または20の境界値を1件以上含む）
...
- `skipped`かつ家族0人で `/pantry`、`/history`、`/shopping`、`/settings`、`/emergency-menus` を直接開き、onboardingへ戻されず、例外なく理解可能な空状態を表示し、不適切な家族安全requestを送らないこと。緊急献立は下書きなしとidea下書きの両方を含む
...
- 同一ownerのprocessing v2に対して別keyのnew/whole/dishを予約し、すべて`generation_in_progress`、外部AI送信0、request・quota・attempt・snapshot増分0であることを確認する。
...
| --- | --- | --- | --- |
| 1 | デザイントークンと共通ウィザード部品 | `.guided-planner-theme`配下に限定してカラートークン、対象画面のボタン、focus、contrast testを追加する。既存global styleを維持し、安全表示の文字/roleとwizard操作44px、カード角丸・shadowを回帰契約に含める。 | なし |
...
| 3 | `TargetMode`、人数、保存スキーマ | clean resetへ共有Zod契約、nullableな下書き、mode条件付きnullableな凍結人数、`menus.target_mode` を直接追加する。`menus.servings`、snapshot、fingerprintはNOT NULLを維持し、アレルゲン版・年齢別ルール版だけをモード条件付きnullableへ変更する。`save_generation_draft`と現行household予約・完了をTask 3→4の開発時境界だけ追随させ、Task単独で現行生成を維持する。 | Task 2 |
| 4 | 生成コマンドv2とrequest snapshot | `request_hmac_version` のDB CHECKと予約RPCをv2専用へ原子的に置換する。owner複合FK、1〜20/空/重複なし制約を持つrequest-bound immutable snapshot、kind/mode別integrity union、ledger-first HMAC replay、v2 pending、source-version fail-closed、owner単位のprocessing制約を実装する。 | Task 3 |
...
| 6 | `/welcome`、献立作成ウィザード、最小結果・履歴境界 | 開始画面、4質問、アイデア人数、生成前確認、実入力単位のfield error関連付け、追加条件、同意往復、対象未選択下書きの対象ステップ再開を実装する。Task 5のサーバー経路が利用可能になってから `RequireCompletedOnboarding` を主要ルートから外す。同じTaskでidea結果と履歴一覧・詳細のmode-aware read-only境界、常時注意、家族/買い物hookの非mountと全shopping request/storage不在、許可操作の非表示を実装し、安全に本文を閲覧できる状態まで完成させる。 | Task 5 |
...
| 8 | セキュリティ統合・全体回帰 | canary、矛盾payload、人数改変、v2同士のprocessing競合、shopping RPC/HTTP/service/browser、source削除後replay、全cross-RPC deadlock race、通信復帰、家族0人の主要route smoke、320px、44px、field error accessibilityを統合検証し、`AGENTS.md` の9段階gateを完走する。 | Task 7 |
