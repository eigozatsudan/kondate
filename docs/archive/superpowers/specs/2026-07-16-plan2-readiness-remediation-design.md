...
これは共通化だけを目的とする整理ではなく、merged Plan 2 に存在する safety fail-open の修正である。現状のUIは `memo` だけを検査し、productionの `netlify/functions/_shared/current-safety.ts` は `context.safety.requestText` を常に空文字にしている。そのため `validateGeneratedMenu` のserver-side unsupported-medical-request検査は全リクエストで実質的にno-opであり、`mainIngredients` や `avoidIngredients` に書かれた離乳食・嚥下・治療食依頼がAI境界へ到達できる。Plan 3着手前に閉じるBlockerとして扱う。
...
この節はcommit `04ba6e0` の自動競合解除を満たされた前提として利用するのではなく、その方式を意図的に置き換える。`04ba6e0` は新しい `initialRevision` が届けば、呼び出し側が表示本文も同じserver rowへ同期したと仮定してconflictを解除する。しかし実routeではTanStack Queryがrefetch中も古いcached rowを保持でき、`initialized` effectがその旧本文を先に再採用した後、遅れて届いた新revisionだけでhookをre-baselineできる。このため永久ロックは解消しても、別タブの最新本文を表示せず旧本文を新revisionで上書きする第二の競合経路が残る。

実装では `04ba6e0` のうち `use-draft-autosave.ts` と `planner-route.tsx` の自動解除部分を作り直す。他の同commit修正は対象にしない。また、単に自動解除を削除して旧永久ロックへ戻すこともしない。明示的なreset tokenと利用者操作を、競合から復帰する唯一の安全な経路として追加する。
