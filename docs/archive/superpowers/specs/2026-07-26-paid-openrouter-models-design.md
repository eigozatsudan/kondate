...
`docs/archive/superpowers/specs/2026-07-11-kondate-mvp-design.md` は次を固定している。
...
| 単価上限 | prompt + completion の 1M 換算和 ≤ **$4.00**（R3 改訂 P*）。request/cache 等は **判定に使わない**（§4.1.7） |
...
7. 同 API の `pricing.prompt` と `pricing.completion` を数値化し、1 トークン単価 × 1e6 した値の **和**が **$4.00 以下**。
   - **ちょうど $4.00 は可**（`sum <= 4`）。
...
（`docs/archive/bugfix/artifacts/r1-higher-tier-decision-record-2026-07-28.md`）は
...
- `docs/archive/superpowers/specs/2026-07-11-kondate-mvp-design.md`（§11.1, §18 および L381 / L383 / L661 / L668 / L678 相当）
...
8. **単価ゲートは prompt+completion のみ、≤ $4.00（境界含む・R3 P*）。request/cache は無視**する。
