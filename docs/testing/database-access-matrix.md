...
| `private.cleanup_expired_shopping_mutations(p_user_id uuid, p_limit integer)` | postgres | none | none | none | n/a | private helper; no browser-role EXECUTE |
...
| `public.cleanup_ai_generation_requests(p_before timestamp with time zone, p_user_id uuid)` | postgres | none | none | EXECUTE | n/a (function) | service_role-only SECURITY DEFINER RPC |
| `public.cleanup_auth_continuations(p_now timestamp with time zone)` | postgres | none | none | EXECUTE | n/a (function) | service_role-only SECURITY DEFINER RPC |
| `public.cleanup_stale_ai_generations(p_now timestamp with time zone)` | postgres | none | none | EXECUTE | n/a (function) | service_role-only SECURITY DEFINER RPC |
