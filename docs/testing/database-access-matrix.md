...
| `private.ai_identity_daily_external_attempts` | postgres | none | none | none | off (not exposed) | identity daily attempt ledger; no user_id FK; service via SECURITY DEFINER only |
...
| `public.acquire_account_delete_lock(p_user_id uuid, p_lock_token text, p_expires_at timestamp with time zone)` | postgres | none | none | EXECUTE | n/a (function) | service_role-only SECURITY DEFINER RPC; account delete lock acquire |
...
| `public.release_account_delete_lock(p_user_id uuid, p_lock_token text)` | postgres | none | none | EXECUTE | n/a (function) | service_role-only SECURITY DEFINER RPC; account delete lock release |
...
| `public.lookup_flyer_weekly(p_user_id uuid, p_idempotency_key text)` | postgres | none | none | EXECUTE | n/a (function) | service_role-only SECURITY DEFINER RPC; flyer succeeded replay without new reserve |
...
| `public.reserve_ai_generation(p_user_id uuid, p_idempotency_key uuid, p_request_kind text, p_draft_id uuid, p_draft_revision bigint, p_source_menu_id uuid, p_replace_dish_id uuid, p_change_reason text, p_request_hmac_version text, p_request_hmac text, p_integrity_context jsonb, p_identity_key text, p_user_limit integer, p_global_limit integer, p_quota_disabled boolean, p_stale_after_seconds integer, p_now timestamp with time zone)` | postgres | none | none | EXECUTE | n/a (function) | service_role-only SECURITY DEFINER RPC |
| `public.reserve_ai_repair_call(p_request_id uuid, p_global_limit integer, p_quota_disabled boolean, p_now timestamp with time zone)` | postgres | none | none | EXECUTE | n/a (function) | service_role-only SECURITY DEFINER RPC |
| `public.reserve_flyer_weekly(p_user_id uuid, p_identity_key text, p_idempotency_key text, p_attempt_limit integer, p_short_window_limit integer, p_global_limit integer, p_quota_disabled boolean, p_now timestamp with time zone)` | postgres | none | none | EXECUTE | n/a (function) | service_role-only SECURITY DEFINER RPC; flyer S0–S4 multi-ledger reserve |
| `public.mark_flyer_weekly_sent(p_request_id uuid, p_now timestamp with time zone)` | postgres | none | none | EXECUTE | n/a (function) | service_role-only SECURITY DEFINER RPC; flyer try/attempt/global sent + short |
| `public.finalize_flyer_weekly_success(p_request_id uuid, p_result jsonb, p_now timestamp with time zone)` | postgres | none | none | EXECUTE | n/a (function) | service_role-only SECURITY DEFINER RPC; flyer success reserved→success |
| `public.stash_flyer_weekly_result(p_request_id uuid, p_result jsonb, p_now timestamp with time zone)` | postgres | none | none | EXECUTE | n/a (function) | service_role-only SECURITY DEFINER RPC; stash validated flyer result on processing row |
| `public.finalize_flyer_weekly_failure(p_request_id uuid, p_failure_code text, p_sent boolean, p_now timestamp with time zone)` | postgres | none | none | EXECUTE | n/a (function) | service_role-only SECURITY DEFINER RPC; flyer S8/S9 release |
| `public.put_weekly_plan_intent(p_request_id uuid, p_user_id uuid, p_snapshot jsonb, p_fingerprint text)` | postgres | none | none | EXECUTE | n/a (function) | service_role-only SECURITY DEFINER RPC; upsert weekly plan intent snapshot for sticky/retry recovery |
| `public.get_weekly_plan_intent(p_request_id uuid, p_user_id uuid)` | postgres | none | none | EXECUTE | n/a (function) | service_role-only SECURITY DEFINER RPC; read weekly plan intent snapshot for the owning user |
...
| `public.set_taste_learning_enabled(p_enabled boolean, p_expected_seq bigint)` | postgres | none | EXECUTE | EXECUTE | n/a (function) | browser-callable SECURITY DEFINER RPC; profiles のテーブル単位 UPDATE を戻さずトグルと比較更新（CAS）用の連番 `taste_learning_seq` だけを書く |
