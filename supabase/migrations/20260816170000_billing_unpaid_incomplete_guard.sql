-- B-R6: B-R5 の unpaid 固定 / kill_source 保護を incoming incomplete にも広げる。
-- incoming incomplete を persist すると kill_source が消え、後続 deleted+canceled を
-- B1 が incomplete_expired に固定して支払済み残存の回復が死ぬ。
-- RLS / GRANT / 列定義は変えない。SECURITY DEFINER 関数の置換のみ。
-- 既存 migration 20260816140000 / 20260816150000 / 20260816160000 は書き換えない。
