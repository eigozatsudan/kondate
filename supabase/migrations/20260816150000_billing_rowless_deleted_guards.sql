-- B-R1/B-R2: 行無しの初回 deleted(canceled) を incomplete_expired 相当にし、
-- 既存の支払済み行は incoming incomplete_expired で潰さない。
...
begin
  ...
  -- B-R1: 行無しの初回 deleted(canceled) は支払証拠が無い。canceled のまま書くと期間内 Plus。
  -- since がある deleted は B-R2 の past_due 残存なので canceled のまま（読取枝が発火する）。
  elsif not v_has_row
     and v_status = 'canceled'
     and v_event_type = 'customer.subscription.deleted'
     and v_past_due_since is null then
    v_status := 'incomplete_expired';
  -- B-R1: webhook が支払証拠の無い deleted を incomplete_expired で送っても、
  -- 既存の支払済み行（active/trialing/canceled/past_due）は canceled 残存のまま。
  elsif v_has_row
