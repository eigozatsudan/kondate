-- R1 (adversarial-quality-pass-1250d8a0 re-review residual):
-- 未承認 remove が残る reconcile では shopping_list_sources への menu version 刻印を延期する。
-- （SHOP2 は add/replace 完全承認を維持。D-C2 で remove は任意のまま。）
-- 旧実装が remove 未選択のまま刻印済みの場合、remove のみの追従 apply は衝突を許す。
-- R2 の current 再投影は service 層の post-apply revalidate で閉じる（本 SQL は stamp のみ）。
