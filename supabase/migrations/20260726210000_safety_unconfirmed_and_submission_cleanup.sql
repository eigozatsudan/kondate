-- A-I3: アレルギー／対象外の未確認メンバーを current safety スナップショットに含める。
-- 件数不一致で status=unavailable → HTTP 500 になると、再検証の allergy_unconfirmed と
-- 緊急献立の current_safety_unavailable がデッドコードになる。生成経路は別途メンバー検査で 422 する。
-- A-I10: generation_draft_submission_versions を captured_at 基準 30 日で掃除（自由入力の死蔵防止）。
...
as $function$
  ...
  eligible_members as (
    ...
    join public.household_members member
      ...
      -- 未確認も載せ、TS 側で allergy_unconfirmed / unsupported_diet を組み立てる（500 にしない）
      and member.allergy_status in ('none', 'registered', 'unconfirmed')
      and member.unsupported_diet_status in ('none', 'present', 'unconfirmed')
  ),
  member_snapshot as (
    select jsonb_agg(
      jsonb_build_object(
...
-- ---------------------------------------------------------------------------
-- A-I10: freeze 提出（memo 等の自由入力）を 30 日で削除
-- FK は ON DELETE CASCADE ではないため、台帳削除後の孤児も captured_at で落とす
-- ---------------------------------------------------------------------------
...
begin
  ...
  with doomed as (
    select draft_id, user_id, draft_revision
    from private.generation_draft_submission_versions
    where captured_at < p_before
    order by captured_at asc, draft_id asc, draft_revision asc
    limit p_limit
