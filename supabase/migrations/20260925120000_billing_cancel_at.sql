-- UX 残り R2 項目 6: Stripe subscription.cancel_at（解約予定の日時）を投影する。
-- Customer Portal や新しい API は解約予約を cancel_at だけで表し、cancel_at_period_end は
-- false のまま来ることがある。画面の「次回の更新日」を「終了予定日」に直すための表示専用の列。
--
-- 変えないもの: 課金の状態判定（entitled・status 遷移・二重 live の cancel・same-second の
-- 終端性・B1/B-R3/B-R5/B-R6 の各ガード）。関数本体は 20260816170000 の
-- project_billing_subscription と 20260814000100 の get_billing_entitlement_for_user を
-- そのまま写し、cancel_at の読み書きだけを足した。billing_entitlement_json（判定の正本）は触らない。
-- RLS / GRANT は変えない（CREATE OR REPLACE は ACL を消さない。private 表の列追加は表の権限を継ぐ）。
...
begin
  ...
  -- R2-6: 表示専用。キー欠落（旧 Function）や null は「予定なし」。判定には使わない
  v_cancel_at := private.billing_payload_timestamptz(p_payload -> 'cancel_at');
  ...
  if not p_force_apply and v_has_row then
    ...
    if v_event_created = v_row.last_stripe_event_created then
      ...
      if v_retrieved is not null then
        ...
        if v_retrieved ? 'cancel_at' then
          v_cancel_at := private.billing_payload_timestamptz(v_retrieved -> 'cancel_at');
        end if;
  ...
  -- B-R5 / B-R6: incoming incomplete / incomplete_expired では unpaid を上げない
  --（unknown-price を Plus にしない）。incomplete* に固定すると B1 が後続
  -- deleted+canceled を飲み、支払済み回復が死ぬ。persist は unpaid のまま。
  -- period / sub も既存の支払済み行を残す。Checkout 放棄は incomplete のまま届き得る。
  elsif v_has_row
     and v_status in ('incomplete', 'incomplete_expired')
  ...
  -- B-R5 / B-R6: persist が unpaid のままなら payload の null で kill_source を消さない。
  -- incoming incomplete でも status を unpaid に戻すので、この枝が発火する。
...
begin
  ...
  return v_json || jsonb_build_object(
    'kill_source_status', v_row.kill_source_status,
    'cancel_at', private.billing_iso_z(v_row.cancel_at)
  );
end;
$function$;
