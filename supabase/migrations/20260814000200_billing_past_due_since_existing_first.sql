-- B-R6: kill unpaid の else が payload 優先だと、後続 past_due / payment_failed が
-- past_due_since を T1 に上書きし、解除後 grace が伸びる。
-- 設計の書込みロックは「初めて past_due」で coalesce(既存, now)。既存優先に揃える。
