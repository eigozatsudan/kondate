-- reserve_ai_generation の再生成ガード: menu_target_members.household_member_id は
-- 家族メンバー削除時に on delete set null で NULL が残る。TS 側
-- resolveGenerationIntegrityContext は NULL リンクを除外して target_member_ids を
-- 署名するため、SQL 側も同じく NULL を除外しないと {uuid,NULL} IS DISTINCT FROM
-- {uuid} で source_menu_changed が恒久的に発生し、生存メンバーを含む献立の
-- 再生成が不可能になる。兄弟関数 confirm_menu_label_confirmation も同規約
-- （NULL リンク = 対象外）で is not null を付けている。
-- 変更点は v_member_ids サブクエリへの `household_member_id is not null` 追加のみ。
-- 署名・security definer・search_path・権限は既存定義をそのまま維持
-- （create or replace は既存の grant を保持する）。
...
begin
  ...
  if p_request_kind = 'new_menu' then
    ...
    insert into private.generation_draft_submission_versions(
      ...
      ingredient_preference, novelty_preference, avoid_ingredients, memo,
      ...
      v_draft.novelty_preference, v_draft.avoid_ingredients, v_draft.memo,
      ...
      (
        ...
        from (
          ...
          where mtm.menu_id = v_menu.id and mtm.user_id = p_user_id
            and mtm.household_member_id is not null
        ) members
      ),
      array[]::uuid[]
    ) into v_member_ids;
