...
test(
  "resumes a partially saved member, shows next-action after complete, then reaches /planner without privacy consent",
  {
    tag: ["@smoke"],
  },
  async ({ authenticatedPage: page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "匿名の緊急候補への協力（任意）" }),
    ).toBeVisible();
