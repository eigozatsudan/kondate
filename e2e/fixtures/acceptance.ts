...
export async function deleteThroughSettings(page: Page): Promise<void> {
  ...
  // 残存データは identity 利用回数 + 無料期間履歴など（「だけ」固定は使わない）
  await expect(danger.getByText(/復元できない識別子[とや]日々の利用回数/u)).toBeVisible();
  await danger.getByRole("button", { name: "削除の確認へ進む" }).click();
  const dialog = page.getByRole("dialog", { name: "アカウントを削除しますか？" });
  await expect(dialog).toBeVisible({ timeout: 10_000 });
  await expect(dialog.getByText(/復元できない識別子[とや]日々の利用回数/u)).toBeVisible();
