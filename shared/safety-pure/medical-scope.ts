...
import { normalizeFoodTextBase } from "./normalize-food-text.js";
...
// Cf/ZWJ すり抜けは detect 前の normalizeFoodTextBase（区切り保持・\p{Cf} 除去）で閉じる。
// ラテン略語は base が lower-case するためパターン側も小文字で合わせる。
...
export function detectUnsupportedMedicalRequest(text: string): readonly UnsupportedDietKind[] {
  // 過剰一般化（英語 renal 等の辞書拡張）はせず、書式制御除去と仮名折り畳みのみ
  const normalized = normalizeFoodTextBase(text);
  return patterns.filter(([, pattern]) => pattern.test(normalized)).map(([kind]) => kind);
}
