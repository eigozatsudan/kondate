...
const EXCLUDED_ALIAS_CONTEXTS = new Map<string, readonly string[]>([
  ...
  [
    "かに",
    [
      // 副詞形「〜かに」は alias の一部として現れるだけで蟹を指さない
      "やわらかに",
      "柔らかに",
      "いかに",
      "たしかに",
  ...
  [
    ...
    [
      ...
      "いかどうか",
      "いかですか",
      // 「いかが」「いかにも」は「柔らかいイカが…」「イカにも火を通す」のように
      // 実際のイカ＋助詞と衝突し得るため、安全側に倒して除外しない
    ],
  ],
  [
    "そば",
    [
      ...
      "そのそばに",
      "そのそばへ",
      // 「のそばで」「そのそばで」は「十割のそばです」「十割のそばで打つ」の prefix に
      // 衝突し得るため、安全側に倒して除外しない
    ],
  ],
  ["もち", ["もちもち食感"]],
  // 2026-09-23: 裸の「鶏」「豚」「牛」を alias に足すにあたり、肉を指さない衝突語を除外する。
  // 迷った語（せせり等）は除外せず安全側（一致させる側）に倒す。
  ["鶏", ["鶏卵"]],
  // 牛刀は調理器具の名称（手順文「牛刀で切る」）。
  ["牛", ["牛乳", "牛蒡", "蝸牛", "水牛", "牛刀"]],
  // 豚カツソースは加工品 alias 側で label 確認つきの一致を別に持つため、ここでは裸の
  // 「豚」の hard 一致から外すだけ。「豚かつソース」も正規化（カタカナ→ひらがな）で
  // 同じ文字列になるため 1 語で両方の表記を覆う。「とんかつソース」は「豚」を含まないので
  // ここには置かない（m3: 以前の登録は効いていなかった）。
  ["豚", ["河豚", "豚カツソース"]],
  // とんかつソースは「とんかつ」alias の hard 一致から外す（B1: 誤検知防止）。
  // 加工品としての一致は下の processed alias（label 確認つき）で別に持つ。
  ["とんかつ", ["とんかつソース"]],
  // ハラミは牛の部位名だが、鮭・サーモンのハラミ（魚の腹身）は牛ではないため除外する。
  ["はらみ", ["鮭ハラミ", "さけハラミ", "サーモンハラミ"]],
  // レバーは手順文の「レバーを引く」やコンロの「レバー式」でも現れる（I3）。
  // I-A: 「レバーをひ」「レバーを引」は「レバーをひと口大に切る」「レバーをひき肉と合わせる」
  // 「レバーを引き上げる」のような本物のレバーの下ごしらえ文まで外したため、
  // 「引いて」「引く」だけに狭める。ひらがなの「ひいて」は「挽いて」とも読めるので除外しない。
  ["ればー", ["ればーを引いて", "ればーを引く", "ればー式"]],
  // ホルモンは医学・健康語としても現れる（I3）。料理の「ホルモン焼き」等は除外しない。
  ["ほるもん", ["女性ホルモン", "成長ホルモン", "ホルモンバランス"]],
  // I-B: 裸の「もつ」（label 確認行）を「日もちする」「保つ」「もつれる」の意から外す。
  // 除外は正規化後の形で持つ（カタカナの「モツ」も正規化でひらがなになる）。
  // 本物のもつの文（「茹でたもつを炒める」「10分ほどもつを煮る」「しょうががもつの臭みを消す」
  // 「翌日もつを茹でる」「もつレタス炒め」）を飲み込まないよう、どの語も前後を狭めている。
  // 「たもつ」→「をたもつ」、「ほどもつ」→「日ほどもつ」、「日もつ」→ 数字＋「日もつ」、
  // 「もつれ」→ 動詞の活用形だけ。「がもつ」は「Xがもつの…」と区別できないため除外しない。
  [
    "もつ",
    [
      "をたもつ",
      "日ほどもつ",
      // 算用数字（NFKC で全角も半角になる）・漢数字・数・何 ＋「日もつ」（2日もつ・数日もつ 等）
      ...[
        "0",
        "1",
        "2",
        ...
        "数",
        "何",
      ].map((numeral) => `${numeral}日もつ`),
      "もつれな",
      "もつれる",
      "もつれて",
    ],
  ],
  // I-C: 一字の「鳥」（chicken）を地名・鳥類・慣用句から外す。
  // 七面鳥・駝鳥・鳥肌 等は迷うため除外しない（一致させる側に倒す）。
  // I1: 地名は「鳥取」だけだと手順文の「焼いた鳥取り出す」「ゆで鳥取り分け」（助詞を省いた
  // 「鳥（を）取り…」）まで飲み込むため、地名の形（県・産・市・砂丘）に狭める。「鳥取の梨」は
  // 「蒸し鳥取のぞく」（取り除く の送り仮名ゆれ）と区別できないため除外せず、誤検知側に倒す。
  // I2: 「鳥貝」（トリ貝）は「蒸し鳥貝割れ巻き」「鳥貝柱スープ」を飲み込むため除外しない。
  // トリ貝の料理が chicken に一致する誤検知は安全側として許容する。
  [
    "鳥",
    [
      "鳥取県",
