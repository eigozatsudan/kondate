...
it("汎用魚語のownerless逆状態を解決済みの全具体魚へ結合する", () => {
  const base = sourceBoundSafetyMenu({ actionIngredient: "salmon", includeSecondFish: true });
  const firstDish = base.dishes[0]!;
  const mackerel = firstDish.ingredients.find((ingredient) => ingredient.name === "鯖")!;
...
it("同じ料理に実食材鯛があっても説明文の鯛焼きを具体魚sourceへ結合しない", () => {
  const base = menuWithNamedIngredient("鯛");
  const menu = makeValidatedMenu({
    ...base,
    dishes: base.dishes.map((dish, index) =>
      index === 0 ? { ...dish, description: "鯛焼きを添える" } : dish,
    ),
  });
  const safety = requiredConstraintContext("remove_bones");
  const issues = evaluateFoodSafetyRules(menu, {
    ...
    foodSafetyRules: safety.foodSafetyRules.map((rule) => ({ ...rule, matchTerms: ["鯛"] })),
  });

  expect(issues.filter((issue) => issue.path === "dishes.0.description")).toEqual([]);
});
...
it("同一節の名前付き別食材の逆状態を全称指示として対象食材へ転嫁しない", () => {
  expect(
    menuWithBoundActionContradiction({
      kind: "cut_small",
      targetName: "ぶどう",
      positiveInstruction: "ぶどうを小さく切る",
      contradictionInstruction: "すべての食材は小さく切るがにんじんは丸ごと盛り付ける",
    }),
  ).toEqual([]);
});

it("全称逆状態を除外対象以外の食材へ適用する", () => {
  expect(
    menuWithBoundActionContradiction({
      kind: "cut_small",
      targetName: "ぶどう",
      positiveInstruction: "ぶどうを小さく切る",
      contradictionInstruction: "すべての食材はにんじんを除いて丸ごと盛り付ける",
    }),
  ).toEqual(
    expect.arrayContaining([expect.objectContaining({ code: "safety_action_contradiction" })]),
  );
